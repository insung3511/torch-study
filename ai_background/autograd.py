...
for t in range(1, 501):
    y_pred = x.mm(w1).clamp(min = 0).mm(w2)         # Predict result number, x & parameter w1 product output

    loss = (y_pred - y).pow(2).sum()                # compare predict output and label output. So I like a y_pred - y_pred^2
